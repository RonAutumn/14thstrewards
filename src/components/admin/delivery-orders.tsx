...
export function DeliveryOrders() {
  ...
  useEffect(() => {
    async function fetchOrders() {
      try {
        ...
        const deliveryOrders = (data.orders || []).map(
          (order: RawOrderData): DeliveryOrder => {
            const parsedItems =
  ...
  const ordersByDate = orders.reduce(
    (acc: { [key: string]: DeliveryOrder[] }, order) => {
      const date =
        order.deliveryDate || format(new Date(order.timestamp), "yyyy-MM-dd");
      if (!acc[date]) {
        acc[date] = [];
      }
      acc[date].push(order);
